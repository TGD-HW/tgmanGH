...
| Current regulation error   | 0x00080000 						   | 19 					 | A deviation between the actual and desired current has been detected.<br>Cause 1: Faulty wiring.<br>Solution 1: Check the wiring and connectors.<br>Cause 2: Incorrect current controller settings or another parameter configuration error.<br>Solution 2: Verify that the current controller parameters are set correctly.<br>Cause 3: TGZ drive hardware fault.<br>Solution 3: Contact the service department of the servo drive supplier. |
...
| DCbus undervoltage warning  | -		                            | Lo C.1, C.2                | DCbus voltage is too low. Warning is active even without drive Enable.<br>Cause 1: DC power supply source is not connected or is in fault state or is off.<br>Solution 1: Check power supply.<br>Cause 2: D-VoltDCLinkMinErrLim parameter is set to high value.<br>Solution 2: Correct D-VoltDCLinkMinErrLim parameter.<br>Cause 3: Short circuit on DC-link.<br>Solution 3: Contact the service department.               |
